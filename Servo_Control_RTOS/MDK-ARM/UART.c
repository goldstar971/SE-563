		...
		// input was x or X, discard previous character input
			...
			//if the two previous characters have been input, check if input is carriage return
				...
				switch(buffer[0]){
					...
					case 'R':
						if(motor1.paused){
							set_motor_position(&motor1, motor1.motor_position - 1,1);
						}
						break;
					case 'n':
					case 'N':
					...
					case 'B':
					case 'b':
						motor_Init(&motor1, 1, motor1.motor_position, motor1.recipe_num);
						motor1.paused = 0;
						break;
					default:
						break;
				...
				switch(buffer[1]){
					...
					case 'R':
						if(motor2.paused){
							set_motor_position(&motor2, motor2.motor_position - 1, 1);
						}
						break;
					case 'n':
					case 'N':
					...
					case 'B':
					case 'b':
						motor_Init(&motor2, 2, motor2.motor_position, motor2.recipe_num);
						motor2.paused = 0;
						break;
					default:
						break;
